...
@interface scoreviewController3 ()
-(void)configureView;
...
@implementation scoreviewController3
...
-(UITextField*)makeTextField:(CGRect)rect text:(NSString*)text{
    ...
    //Diagram *diagram=[[Diagram alloc]initWithFrame:CGRectMake(0,0,10,10)];
    //[textfield addSubview:diagram];
    return textfield;
}
...
-(void)viewWillAppear:(BOOL)animated{
    [self.navigationController setToolbarHidden:NO animated:NO];//必要
    self.navigationController.navigationBarHidden=NO;
    
    [self makeNumberButton];
    
    /*MPMediaItem *nowPlayingItem = [player nowPlayingItem];
    NowPlayingItem=[nowPlayingItem valueForProperty:MPMediaItemPropertyTitle];
    player = [MPMusicPlayerController applicationMusicPlayer];
    [player setQueueWithItemCollection:_Collection];
    metronome=NO;
    metronome3=NO;
    MediaPlayer=NO;*/
    [self configureView];
...
- (void)viewDidLoad{
    ...
    //スクロールビュー作成
    score=[[UIView alloc]init];
    scroll=[[UIScrollView alloc]initWithFrame:self.view.bounds];//frameだと回転したときにずれる
    ...
    layer.position=CGPointMake(self.view.frame.size.width/2,3800);
    [scroll addSubview:score];
    scroll.contentSize=CGSizeMake(0,7700);
    [self.view addSubview:scroll];
    /*score.translatesAutoresizingMaskIntoConstraints = NO;
    NSLayoutConstraint *scoreLeftConstraint = [NSLayoutConstraint constraintWithItem:score
                                                                          ...
                                                                          multiplier:1
                                                                            constant:0];
    NSLayoutConstraint *scoreTopConstraint = [NSLayoutConstraint constraintWithItem:score
                                                                         ...
                                                                         multiplier:1
                                                                           constant:0];
    NSLayoutConstraint *scoreHeightConstraint = [NSLayoutConstraint constraintWithItem:score
                                                                            ...
                                                                            multiplier:1
                                                                              constant:7100];
    NSLayoutConstraint *scoreWidthConstraint = [NSLayoutConstraint constraintWithItem:score
    ...
    [scroll addConstraints:@[scoreLeftConstraint, scoreTopConstraint, scoreHeightConstraint ,scoreWidthConstraint ]];*/
    ...
    UIBarButtonItem *cancel=[[UIBarButtonItem alloc]initWithTitle:NSLocalizedString(@"Back", nil)style:UIBarButtonItemStylePlain target:self action:@selector(torikeshi:)];
    ...
    /*UIBarButtonItem *media=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:NSLocalizedString(@"Music", nil)] style:UIBarButtonItemStylePlain target:self action:@selector(Media:)];
     media.tintColor=[UIColor colorWithRed:2/255.0 green:31/255.0 blue:140/255.0 alpha:1];*/
    
    items1=[NSArray arrayWithObjects:tempolabel,spacer,copy,spacer,rewind,spacer,play,spacer,nil];
    items2=[NSArray arrayWithObjects:spacer,spacer,rewind,spacer,stop,spacer,nil];
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textField1];textField1.tag=(i*4)+1;
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textField2];textField2.tag=(i*4)+2;
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textField3];textField3.tag=(i*4)+3;
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textField4];textField4.tag=(i*4)+4;
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textview1];textview1.tag=(i*4)+201;
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textview2];textview2.tag=(i*4)+202;
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textview3];textview3.tag=(i*4)+203;
    ...
    for (int i=0; i<=49; i++) {
        ...
        [score addSubview:textview4];textview4.tag=(i*4)+204;
    ...
    ButtonPopTipView=NO;
    CC=YES;DD=NO;EE=NO;FF=NO;GG=NO;AA=NO;BB=NO;activeButton2tag=NO;activeButton2tag2=NO;ChordCopy=NO;LyricsCopy=NO;
    metronome4=YES;tempo=NO;Play2=NO;Pause=NO;FirstResponder=NO;
...
-(void)TrackingManager{
    ...
    switch (colorvalue4) {
        case 0:[TrackingManager sendEventTracking:@"detailItem" action:@"Lyrics　Fontdata"label:@"Apple SD Gothic Neo" value:nil screen:screenName];
            break;
        ...
        case 2:
            [TrackingManager sendEventTracking:@"detailItem" action:@"Lyrics　Fontdata"label:@"Hiragino Kaku Gothic ProN" value:nil screen:screenName];
            break;
        case 3:
...
-(void)maketempo{
    switch (value3) {
        case 0:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"3/4  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        case 1:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"4/4  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        case 2:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"2/4  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        case 3:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"5/4  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        case 4:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"6/8  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        case 5:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"12/8  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        case 6:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"2/2  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        case 7:
            tempolabel=[[UIBarButtonItem alloc]initWithTitle:[NSString stringWithFormat:@"4/2  %d",(int)(value+50)] style:UIBarButtonItemStylePlain target:self action:@selector(tempo1:)];
            break;
        default:
            break;
    }
    ...
    if (self.detailItem.colordata!=nil) {
        ...
        for (int i=200; i<=399; i++) {
            ...
            switch (colorvalue3) {
                case 0:textField0.textColor=[UIColor blackColor];break;
                case 1:textField0.textColor=[UIColor redColor];break;
                case 2:textField0.textColor=[UIColor colorWithRed:170/255.0 green:12/255.0 blue:10/255.0 alpha:1];break;
                case 3:textField0.textColor=[UIColor colorWithRed:200/255.0 green:153/255.0 blue:50/255.0 alpha:1];break;
                case 4:textField0.textColor=[UIColor colorWithRed:255/255.0 green:110/255.0 blue:0/255.0 alpha:1];break;
                case 5:textField0.textColor=[UIColor darkGrayColor];break;
                case 6:textField0.textColor=[UIColor colorWithRed:1/255.0 green:31/255.0 blue:141/255.0 alpha:1];break;
                case 7:textField0.textColor=[UIColor colorWithRed:0/255.0 green:142/255.0 blue:42/255.0 alpha:1];break;
                case 8:textField0.textColor=[UIColor colorWithRed:105/255.0 green:130/255.0 blue:27/255.0 alpha:1];break;
                case 9:textField0.textColor=[UIColor colorWithRed:85/255.0 green:32/255.0 blue:142/255.0 alpha:1];break;
                case 10:textField0.textColor=[UIColor brownColor];break;
                case 11:textField0.textColor=[UIColor colorWithRed:213/255.0 green:100/255.0 blue:143/255.0 alpha:1];break;
                case 12:textField0.textColor=[UIColor colorWithRed:255/255.0 green:45/255.0 blue:142/255.0 alpha:1];break;
                case 13:textField0.textColor=[UIColor colorWithRed:255/255.0 green:145/255.0 blue:51/255.0 alpha:1];break;
                case 14:textField0.textColor=[UIColor colorWithRed:213/255.0 green:100/255.0 blue:143/255.0 alpha:1];break;
                case 15:textField0.textColor=[UIColor colorWithRed:151/255.0 green:12/255.0 blue:10/255.0 alpha:1];break;
                case 16:textField0.textColor=[UIColor colorWithRed:255/255.0 green:45/255.0 blue:142/255.0 alpha:1];break;
            ...
            if (colorvalue4==0) {textField0.font=[UIFont fontWithName:@"Apple SD Gothic Neo" size:15];
                if(textField0.text.length>=20) {textField0.font=[UIFont fontWithName:@"Apple SD Gothic Neo" size:13];}
                //アルファベットは１バイトなので20文字目から小さくする。
            }
            else if(colorvalue4==1){textField0.font=[UIFont fontWithName:@"Heiti TC" size:15];
                if(textField0.text.length>=20) {textField0.font=[UIFont fontWithName:@"Heiti TC" size:13];}
                if(bytes>=51) {textField0.font=[UIFont fontWithName:@"Heiti TC" size:13];}
                //日本語は一文字３バイトなので１８文字目から小さくする。
            }
            else if(colorvalue4==2){textField0.font=[UIFont fontWithName:@"Hiragino Kaku Gothic ProN" size:15];
                if(textField0.text.length>=20) {textField0.font=[UIFont fontWithName:@"Hiragino Kaku Gothic ProN" size:11];}
                if(bytes>=51) {textField0.font=[UIFont fontWithName:@"Hiragino Kaku Gothic ProN" size:11];}
            }
            else if(colorvalue4==3){
                if(bytes>=51) {textField0.font=[UIFont fontWithName:@"Hiragino Mincho ProN" size:11];}
                else if(textField0.text.length>=20) {textField0.font=[UIFont fontWithName:@"Hiragino Mincho ProN" size:11];}
                else{textField0.font=[UIFont fontWithName:@"Hiragino Mincho ProN" size:15];}
            }
            else if(colorvalue4==4){textField0.font=[UIFont fontWithName:@"EuphemiaUCAS-Italic" size:15];
                if(textField0.text.length>=20) {textField0.font=[UIFont fontWithName:@"EuphemiaUCAS-Italic" size:11];}
                if(bytes>=51) {textField0.font=[UIFont fontWithName:@"EuphemiaUCAS-Italic" size:11];}
            }
            else if(colorvalue4==5){textField0.font=[UIFont fontWithName:@"Courier" size:15];
                if(textField0.text.length>=20) {textField0.font=[UIFont fontWithName:@"Courier" size:13];}
                if(bytes>=51) {textField0.font=[UIFont fontWithName:@"Courier" size:13];}
            }
            else if(colorvalue4==6){textField0.font=[UIFont fontWithName:@"Futura-MediumItalic" size:15];
                if(textField0.text.length>=20) {textField0.font=[UIFont fontWithName:@"Futura-MediumItalic" size:12];}
                if(bytes>=51) {textField0.font=[UIFont fontWithName:@"Futura-MediumItalic" size:12];}
            }
        }
    }
    //self.detailItem.colordataがない場合
    else{
        //黒とApple SD Gothic Neoに設定
        colorvalue1=0;
        colorvalue2=0;
        colorvalue3=0;
        colorvalue4=0;
        ...
        if (GuitarDiagram || GuitarDiagramLite) {
            ...
            if (isJapanese==YES) {
                ...
                for (int i=0; i<=199; i++) {
                    UITextField *textField0=[textFields objectAtIndex:i];
                    textField0.font=[UIFont fontWithName:@"Chord-Diagram" size:41];
                }
            }
            else{
                colorvalue2=8;
                for (int i=0; i<=199; i++) {
                    UITextField *textField0=[textFields objectAtIndex:i];
                    textField0.font=[UIFont fontWithName:@"Chord-Diagram2" size:41];
                }
            }
        }
        if(UkurereDiagram || UkurereDiagramLite){
            if (isJapanese==YES) {
                ...
                for (int i=0; i<=199; i++) {
                    UITextField *textField0=[textFields objectAtIndex:i];
                    textField0.font=[UIFont fontWithName:@"Ukurere-Diagram" size:41];
                }
            }
            else{
                colorvalue2=11;
                for (int i=0; i<=199; i++) {
                    UITextField *textField0=[textFields objectAtIndex:i];
                    textField0.font=[UIFont fontWithName:@"Ukurere-Diagram2" size:41];
                }
            }
        }
    }
...
-(void)makeFont{
    for (int i=0; i<=199; i++) {
        ...
        if(GuitarDiagram&&UkurereDiagram){
            ...
            else if(colorvalue2==7){textField0.font=[UIFont fontWithName:@"Chord-Diagram" size:41];}
            else if(colorvalue2==8){textField0.font=[UIFont fontWithName:@"Chord-Diagram2" size:41];}
            else if(colorvalue2==9){textField0.font=[UIFont fontWithName:@"Ukurere-Diagram" size:41];}
            else if(colorvalue2==10){textField0.font=[UIFont fontWithName:@"Ukurere-Diagram2" size:41];}
            MutableString=[NSMutableString stringWithFormat:@"%@",[dataarray2 objectAtIndex:i]];
            for (int x=0; x<=MutableString.length; x++) {
                NSRange b=[MutableString rangeOfString:@"b"options:0 range:NSMakeRange(x, MutableString.length-x)];
                NSRange c=[MutableString rangeOfString:@"c"options:0 range:NSMakeRange(x, MutableString.length-x)];
...
-(void)makeKeyboard1{
    ...
    for (int i=0; i<52; i++) {
        ...
        if (colorvalue2==7) {[compButton.titleLabel setFont:[UIFont fontWithName:@"Chord-Diagram" size:30]];}
        else if(colorvalue2==8){[compButton.titleLabel setFont:[UIFont fontWithName:@"Chord-Diagram2" size:30]];}
        else if(colorvalue2==10){[compButton.titleLabel setFont:[UIFont fontWithName:@"Ukurere-Diagram" size:30]];}
        else if(colorvalue2==11){[compButton.titleLabel setFont:[UIFont fontWithName:@"Ukurere-Diagram2" size:30]];}
...
-(void)PickerViewControllerDelegateDidfinish:(NSInteger)getData{
    ...
    switch (value1) {
        case 0:
            tempolabel.title=[NSString stringWithFormat:@"3/4  %d",(int)(value2+50)];
            break;
        case 1:
            tempolabel.title=[NSString stringWithFormat:@"4/4  %d",(int)(value2+50)];
            break;
        case 2:
            tempolabel.title=[NSString stringWithFormat:@"2/4  %d",(int)(value2+50)];
            break;
        case 3:
            tempolabel.title=[NSString stringWithFormat:@"5/4  %d",(int)(value2+50)];
            break;
        case 4:
            tempolabel.title=[NSString stringWithFormat:@"6/8  %d",(int)(value2+50)];
            break;
        case 5:
            tempolabel.title=[NSString stringWithFormat:@"12/8  %d",(int)(value2+50)];
            break;
        case 6:
            tempolabel.title=[NSString stringWithFormat:@"2/2  %d",(int)(value2+50)];
            break;
        case 7:
            tempolabel.title=[NSString stringWithFormat:@"4/2  %d",(int)(value2+50)];
            break;
        default:
            break;
    }
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)sender {
    ...
    //navBarLeftButtonPopTipView = nil;
    ...
    if (GuitarDiagram&&UkurereDiagram) {
        ...
        if (colorvalue2>6) {
            for (int i=0; i<156; i++) {
                UIButton *replaceButtons=[compButtons objectAtIndex:i];
                if (colorvalue2==7) {[replaceButtons.titleLabel setFont:[UIFont fontWithName:@"Chord-Diagram" size:30]];}
                else if(colorvalue2==8){[replaceButtons.titleLabel setFont:[UIFont fontWithName:@"Chord-Diagram2" size:30]];}
                else if(colorvalue2==9){[replaceButtons.titleLabel setFont:[UIFont fontWithName:@"Ukurere-Diagram" size:30]];}
                else if(colorvalue2==10){[replaceButtons.titleLabel setFont:[UIFont fontWithName:@"Ukurere-Diagram2" size:30]];}
            }
        ...
        if (colorvalue2>8) {
            for (int i=0; i<156; i++) {
                UIButton *replaceButtons=[compButtons objectAtIndex:i];
                if(colorvalue2==9){[replaceButtons.titleLabel setFont:[UIFont fontWithName:@"Ukurere-Diagram" size:30]];}
                else if(colorvalue2==10){[replaceButtons.titleLabel setFont:[UIFont fontWithName:@"Ukurere-Diagram2" size:30]];}
            }
            sender.inputView=accessoryView;
        }
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    CGPoint scrollPoint = CGPointMake(0,y-152);
    [scroll setContentOffset:scrollPoint animated:YES];
    CGRect accessFrame=CGRectMake(0, 0, 0, 30);
    UIView *accessoryView3=[[UIView alloc]initWithFrame:accessFrame];
    UIToolbar *toolbar1=[[UIToolbar alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 32)];
    ...
    UIBarButtonItem *spacer=[[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    UIBarButtonItem *next=[[UIBarButtonItem alloc]initWithTitle:@" → "style:UIBarButtonItemStylePlain target:self action:@selector(next2:)];
    next.tintColor=[UIColor colorWithRed:2/255.0 green:31/255.0 blue:140/255.0 alpha:1];
    UIBarButtonItem *back=[[UIBarButtonItem alloc]initWithTitle:@" ← "style:UIBarButtonItemStylePlain target:self action:@selector(back3:)];
    back.tintColor=[UIColor colorWithRed:2/255.0 green:31/255.0 blue:140/255.0 alpha:1];
    NSArray *items3=[NSArray arrayWithObjects:spacer,spacer,spacer,back,next,Done,nil];
...
-(void)Metronome{
    ...
    while (Play) {
        ...
        if (metronome3) {//tickに行く前にYESであればscrollを画面上部に移動させる
            if (Play2==NO) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    CGPoint scrollPoint = CGPointMake(0,9);
                    [scroll setContentOffset:scrollPoint animated:YES];
                });
            }
    ...
    if (barcount3<=beat+3) {
        ...
        switch (barcount2) {
            case 0:{
                ...
                NSLog(@"y,%f",y);
                //NSLog(@"numberButton,%f",numberButton.frame.origin.x);
                if (numberButton.tag<403) {}
                else if (x<numberButton.frame.origin.x) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        CGPoint scrollPoint = CGPointMake(0,y-50);
...
-(void)PlayAnimation:(BOOL)aPlay{
    if (aPlay) {
        ...
        //アニメーションの開始位置。ナビゲーションバーを消すため少し下げてからスタート。6837 7803 7600
        animation.fromValue=[NSValue valueWithCGPoint:CGPointMake(self.view.frame.size.width/2,3952)];//layer.position];
        animation.toValue=[NSValue valueWithCGPoint:CGPointMake(self.view.frame.size.width/2,-3800)];
        animation.duration=bpm2*beat*204;
...
-(void)back:(id)sender{
    if ([[[UIDevice currentDevice]systemVersion]floatValue]>=7) {
        //UITextRange *textRange=[activeField.selectedTextRange];
...
-(void)tempo1:(id)sender{
    ...
    //[TrackingManager sendEventTracking:@"Button" action:@"ShowPicker"label:@"ShowPicker1" value:nil screen:screenName];
}

-(void)Textviewclose:(id)sender{
    [activeField2 resignFirstResponder];
    /*float y = activeField2.frame.origin.y;
    CGPoint scrollPoint = CGPointMake(0,y-180);
    [scroll setContentOffset:scrollPoint animated:YES];*/
...
/*-(void)Media:(id)sender{
    ...
    [self.navigationController pushViewController:mediaplayer animated:NO];
}*/

-(void)Edit:(id)sender{
    [self dismiss];
    infomationViewController2 *info2=[[infomationViewController2 alloc]init];
    info2.detailItem=self.detailItem;
    [self save];
    ...
    if (ButtonPopTipView==NO){
        if (activeButton2tag==NO) {
            if (activeButton2tag2==NO) {
                ...
                beetViewController.detailItem=self.detailItem;
                ...
                //NSLog(@"%lu",(unsigned long)presentationController.permittedArrowDirections);
                presentationController.delegate=self;
                presentationController.backgroundColor=[UIColor lightGrayColor];
                presentationController.sourceView = score;
